...
function PreviewContent() {
  ...
  if (mainEvent?.date) {
    targetDate = new Date(`${mainEvent.date}T${mainEvent.time}:00`);
  }

  const weddingData = {
    groom: values.groomName || 'Romeo',
    groomParents: values.groomParents || '-',
    bride: values.brideName || 'Juliet',
    brideParents: values.brideParents || '-',
    eventDate: new Date(values.events?.[0]?.date),
    photos: values.gallery?.map((g) => g.imageUrl) || [],
  };

  const coverData = {
    prefixTitle: values.prefixTitle || 'The Wedding of',
    coverGreeting: values.coverGreeting || 'With Love,',
    coverQuote:
      values.coverQuote ||
      'Two souls with but a single thought, two hearts that beat as one.',
    heroTitle: values.heroTitle || 'We Are Getting Married',
  };

  const greetingData = {
    openingGreeting: values.openingGreeting || 'Bismillahirrahmanirrahim',
    openingMessage:
      values.openingMessage ||
  ...
  const closingData = {
    ...
    closingGreeting:
      values.closingGreeting ||
      'Atas kehadiran dan doa restunya, kami ucapkan terima kasih.',
  };

  const guestName = values.recipientName || 'Tamu Undangan';
  ...
  const getYoutubeId = (url: string) => {
    // Regex komprehensif untuk menangkap ID dari berbagai format YouTube (watch, embed, youtu.be, shorts, live)
    const regExp =
              ...
              <section className="bg-primary-subtle relative overflow-hidden px-6 pt-24 pb-16">
                ...
                <div className="relative mx-auto max-w-4xl">
                  ...
                  <p className="text-muted-foreground mb-8 text-center">
                    {formatDate(weddingData.eventDate)}
                  </p>
              ...
              <section className="px-6 py-16">
                <div className="mx-auto max-w-4xl">
                  ...
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    {values.events?.map((evt, idx) => (
                      <PreviewEventCard
                        key={idx}
                        title={evt.title || '-'}
                        date={formatDate(evt.date || new Date().toISOString())}

...
import { useGetPublicInvitationBySlug } from '@/hooks/api/use-invitation';
import { useUpdatePublicGuestStatus } from '@/hooks/api/use-invitation-guest';
import { formatDate } from '@/lib/date';
import { AxiosError } from 'axios';
import { AnimatePresence, motion } from 'framer-motion';
import { Heart, Loader2, Music, OctagonX, VolumeX } from 'lucide-react';
...
function InviteContent({ slug }: { slug: string }) {
  const searchParams = useSearchParams();

  const { data: values, isLoading } = useGetPublicInvitationBySlug(slug);
  const { mutate: updateGuestStatus } = useUpdatePublicGuestStatus();
  ...
  const [isGuestValid, setIsGuestValid] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    if (values && values.status === InvitationStatusEnum.Published) {
      ...
      if (guestName) {
        ...
        setTimeout(() => setIsGuestValid(false), 0);
      }
    }
  }, [values, searchParams, updateGuestStatus]);

  if (isLoading || !values || isGuestValid === null) {
    return (
  ...
  if (isGuestValid === false) {
    return (
      <div
        className="bg-background flex min-h-screen w-full items-center justify-center p-6"
        style={templateObj.cssVars as React.CSSProperties}
      >
        <div className="bg-card text-card-foreground border-border max-w-md rounded-2xl border p-8 text-center shadow-lg">
          <OctagonX className="text-destructive mx-auto mb-6 h-12 w-12 opacity-50" />
          <h2 className="mb-4 font-serif text-2xl font-bold">Akses Ditolak</h2>
          <p className="text-muted-foreground">
            Maaf, nama Anda tidak ditemukan dalam daftar tamu undangan ini.
            Silakan periksa kembali tautan (URL) yang diberikan oleh tuan rumah.
          </p>
        </div>
      </div>
    );
  ...
  if (values.status === InvitationStatusEnum.Draft) {
    return (
      <div
        className="bg-background flex min-h-screen w-full items-center justify-center p-6"
        style={templateObj.cssVars as React.CSSProperties}
      >
        <div className="bg-card text-card-foreground border-border max-w-md rounded-2xl border p-8 text-center shadow-lg">
          <Heart className="text-primary mx-auto mb-6 h-12 w-12 opacity-50" />
          <h2 className="mb-4 font-serif text-2xl font-bold">
            Undangan Belum Tersedia
          </h2>
          <p className="text-muted-foreground">
            Undangan ini masih berstatus draf dan belum dipublikasikan. Silakan
            hubungi calon mempelai untuk informasi lebih lanjut.
          </p>
        </div>
      </div>
    );
  ...
  if (values.status === InvitationStatusEnum.Expired) {
    return (
      <div
        className="bg-background flex min-h-screen w-full items-center justify-center p-6"
        style={templateObj.cssVars as React.CSSProperties}
      >
        <div className="bg-card text-card-foreground border-border max-w-md rounded-2xl border p-8 text-center shadow-lg">
          <Heart className="text-primary fill-primary mx-auto mb-6 h-12 w-12 opacity-50" />
          <h2 className="mb-4 font-serif text-2xl font-bold">
            Undangan Kadaluarsa
          </h2>
          <p className="text-muted-foreground">
            Masa aktif undangan ini telah berakhir. Terima kasih atas
            partisipasi Anda.
          </p>
        </div>
      </div>
...
export default function InvitePage({
  ...
  return (
    ...
    >
      <InviteContent slug={resolvedParams.slug} />
    </Suspense>
  );
}

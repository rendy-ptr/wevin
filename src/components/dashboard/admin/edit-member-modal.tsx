...
export default function EditMemberModal({
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<Omit<CreateUpdateMemberFormValues, 'email'>>({
  ...
  const onSubmit = (data: Omit<CreateUpdateMemberFormValues, 'email'>) => {
    mutate(
      { id: member.id, ...data },
      {
        onSuccess: (res) => {
            ...
            <div className="space-y-2">
              ...
              <div className="relative opacity-60">
                ...
                <Input
                  ...
                  className="bg-secondary/10 border-border/40 h-11 cursor-not-allowed pl-10 text-sm transition-all"
                />
              </div>
            </div>
